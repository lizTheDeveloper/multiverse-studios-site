...
(function initMatrixAuth() {
  ...
  // Derive API base from the current script's path so fetch() works
  // behind a reverse-proxy path prefix (e.g. /precursors/).
  var _scriptEl = document.currentScript;
  var _apiBase = '';
  if (_scriptEl && _scriptEl.src) {
    var _m = new URL(_scriptEl.src).pathname.match(/^(\/[^/]+)\//);
    if (_m) _apiBase = _m[1];
  }
  ...
  async function validateSession() {
    const res = await fetch(_apiBase + '/api/auth/session', {
      method: 'GET',
      credentials: 'same-origin',
    });
    const json = await res.json();
  ...
  async function getOpenIdToken() {
    ...
    const res = await fetch(_apiBase + '/api/auth/openid', {
      method: 'POST',
      credentials: 'same-origin',
    });
    const json = await res.json();
...
`;
  ...
  async function requestPasswordReset(username) {
    const res = await fetch(_apiBase + '/api/auth/request-reset', {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
