...
(function initMatrixAuth() {
  ...
  // Access token was previously kept in sessionStorage (cleared on tab/browser
  // close) to reduce the XSS exfiltration window. Moved to localStorage so
  // users stay signed in across tabs and browser restarts — the token is still
  // readable by any JS on this origin either way, and there is no PII at risk
  // in a game context. The preferred long-term fix remains HttpOnly session
  // cookies backed by a server-side auth proxy.
  //
  // Migration: read from sessionStorage on first load so existing sessions
  // survive the transition, then persist to localStorage going forward.
  ...
  function sessionSet(key, value) {
    try {
      ...
      // Clean up old sessionStorage entry if it exists
  ...
  // ── Session Expiry & Background Validation ────────────────

  var TOKEN_CHECK_INTERVAL = 5 * 60 * 1000; // 5 minutes
  ...
  function handleSessionExpired() {
    ...
    updateNavUI();
    showModal();
    window.dispatchEvent(new CustomEvent('matrixAuthSessionExpired'));
  }
  ...
  function startTokenCheck() {
    ...
    tokenCheckTimer = setInterval(async function () {
      if (!session.accessToken) { stopTokenCheck(); return; }
      try {
  ...
  async function validateSession() {
    ...
    try {
      ...
      if (err.message === 'SESSION_EXPIRED') {
        // handleSessionExpired already called by matrixFetch
        return false;
      }
      clearSession();
      return false;
...
`;
  ...
  document.body.appendChild(overlay);
    // Re-append to end of body so it paints on top of play-gate (which is
    // appended later during init and creates a backdrop-filter stacking context
    // that can cause z-index ordering to break in Blink/WebKit).
    document.body.appendChild(overlay);
