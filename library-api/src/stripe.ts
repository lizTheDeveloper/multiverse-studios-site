...
import { GameProduct, resolveProduct } from './products.js';
 ...
 */
async function getSucceededChargesForCustomer(
  stripe: Stripe,
  customerId: string,
): Promise<Stripe.Charge[]> {
  const results: Stripe.Charge[] = [];
...
async function resolveProductFromCharge(
    ...
    try {
      ...
      for (const line of invoice.lines.data) {
        ...
        if (price && typeof price.product === 'object' && price.product !== null) {
          ...
          const match = resolveProduct(product.id, product.name);
          if (match) return match;
        } else if (price && typeof price.product === 'string') {
          const match = resolveProduct(price.product, line.description);
          if (match) return match;
        }
      }
    } catch {
  ...
  if (charge.payment_intent) {
    ...
    try {
      ...
      const fromPiMeta = resolveProduct(pi.metadata?.product_id, pi.description);
      if (fromPiMeta) return fromPiMeta;
    } catch {
      // Best-effort.
    }
  }
 ...
 */
export async function lookupPurchasesByEmail(email: string): Promise<PurchaseRecord[]> {
  ...
  const best = new Map<string, PurchaseRecord>();
